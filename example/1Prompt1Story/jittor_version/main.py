...
# JDiffusion.utils.logging.set_verbosity_error()
...
def generate_images(unet_controller: UNetController, pipe, id_prompt, frame_prompt_list, save_dir, window_length, seed, verbose=True):
    generate = jtorch.Generator().manual_seed(seed)
...
def main(device, model_path, save_dir, id_prompt, frame_prompt_list, precision, seed, window_length):
    pipe, _ = utils.load_pipe_from_path(model_path, device, jtorch.float16 if precision == "fp16" else jtorch.float32, precision)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--precision', type=str, choices=["fp16", "fp32"], default="fp32", help='Model precision')
