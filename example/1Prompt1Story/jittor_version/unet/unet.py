...
import jittor_version.unet.utils as utils
# SDXL
...
class Timesteps(nn.Module):
    ...
    def forward(self, timesteps):
        ...
        exponent = -math.log(10000) * jtorch.arange(
            half_dim, dtype=jtorch.float32, device=timesteps.device
        )
        exponent = exponent / (half_dim - 0.0)

        emb = jtorch.exp(exponent)
        emb = timesteps[:, None].float() * emb[None, :]

        sin_emb = jtorch.sin(emb)
        cos_emb = jtorch.cos(emb)
        emb = jtorch.cat([cos_emb, sin_emb], dim=-1)
...
class Attention(nn.Module):
    def __init__(
        ...
        self.to_out = nn.ModuleList(
            [nn.Linear(inner_dim, inner_dim), nn.Dropout(dropout,)]
        )
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, unet_controller: Optional[UNetController] = None):
        ...
        if (unet_controller is not None and unet_controller.Use_ipca and unet_controller.current_unet_position in unet_controller.Ipca_position
            ...
            if unet_controller.do_classifier_free_guidance is True:
                scores = jtorch.matmul(q, k.transpose(-2, -1)) * self.scale
                attn_weights = jtorch.nn.softmax(scores, dim=-1)  # this is only used by cross_attn_map store
            ...
            scores = jtorch.matmul(q, k.transpose(-2, -1)) * self.scale
            attn_weights = jtorch.nn.softmax(scores, dim=-1)
            attn_output = jtorch.matmul(attn_weights, v)
...
class GEGLU(nn.Module):
    ...
    def forward(self, x):
        ...
        return x1 * jtorch.nn.functional.gelu(x2)
...
class CrossAttnUpBlock2D(nn.Module):
    ...
    def forward(
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            ...
            hidden_states = jtorch.cat([hidden_states, res_hidden_states], dim=1)
            hidden_states = resnet(hidden_states, temb)
            hidden_states = attn(
                hidden_states,
                encoder_hidden_states=encoder_hidden_states,
...
class UNet2DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        add_embeds = jtorch.concat([text_embeds, time_embeds], dim=-1)
