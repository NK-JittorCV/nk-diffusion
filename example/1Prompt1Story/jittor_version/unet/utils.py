...
from JDiffusion import AutoencoderKL
...
import numpy as np
import jittor_version.unet.pipeline_stable_diffusion_xl as pipeline_stable_diffusion_xl
from torch.fft import fftn, fftshift, ifftn, ifftshift
from typing import Optional, Tuple
import jittor as jt

from jittor_version.unet.unet import UNet2DConditionModel
from jittor_version.unet.unet_controller import UNetController
...
def ipca(q, k, v, scale, unet_controller: Optional[UNetController] = None): # eg. q: [4,20,1024,64] k,v: [4,20,77,64] 
    ...
    attn_weights_neg = torch.nn.softmax(scores_neg, dim=-1)
    ...
    if unet_controller is not None:
        ...
        attn_weights_pos = torch.nn.softmax(scores_pos + torch.log(mask), dim=-1)

    else:
        scores_pos = torch.matmul(q_pos, k_plus) * scale
        attn_weights_pos = torch.nn.softmax(scores_pos, dim=-1)
...
def ipca2(q, k, v, scale, unet_controller: Optional[UNetController] = None): # eg. q: [4,20,1024,64] k,v: [4,20,77,64] 
    ...
    if unet_controller.Store_qkv is True:
        ...
        attn_weights = torch.nn.softmax(scores, dim=-1)
...
def apply_embeds_mask(unet_controller, dropout_mask, add_eot=False):
    ...
    mask_indices = jt.arange(dropout_mask.shape[-1])
    index_list_jt = jt.array(index_list)

    if index_list_jt.numel() == 0:
        is_in_condition = jt.zeros_like(mask_indices, dtype=jt.bool)
    else:
        is_in_condition = (mask_indices.unsqueeze(1) == index_list_jt.unsqueeze(0)).any(dim=1)
    
    not_isin_condition = jt.logical_not(is_in_condition)
    
    mask = (mask_indices >= 78) & not_isin_condition
    dropout_mask[0, :, mask] = 0
    
    # 如果函数需要返回修改后的 dropout_mask，您可以取消下面这行的注释
    # return dropout_mask
...
def swr_single_prompt_embeds(swr_words,prompt_embeds,prompt,tokenizer,alpha=1.0, beta=1.2, zero_eot=False):
    ...
    wo_batch = punish_wight(wo_batch.transpose().to(float), wo_batch.size(0), alpha=alpha, beta=beta, calc_similarity=False).transpose().to(prompt_embeds.dtype)
